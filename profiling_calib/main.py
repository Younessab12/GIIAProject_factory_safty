...
class Calib:
  ...
  def process(self, frame, holistic_res):
    ...
    if self.state!=self.prevState:
      msg = self.messages[self.state]+". if you are ready press on ok"
      sg.popup_ok(msg,title="Factory Safty")
  ...
  def train_model(self):
    ...
    for (i, imagePath) in enumerate(imagePaths):
        ...
        print("[INFO] processing image {}/{}".format(i + 1,
            len(imagePaths)))
        progrees=sg.one_line_progress_meter(
            'Progress Meter', i, len(imagePaths),
            'Training...'
                )
        if not progrees:
          res=sg.popup_ok_cancel("are you sure you want to stop the traing process?")
          if str(res).upper()=="OK":
            sys.exit(0)
...
name = ""
name = sg.popup_get_text("please enter your name",title="factory Safty")
if name =="" or name is None:
  sg.popup_timed("we can't processed because you did not provide us with a name.",auto_close=10,title="Factory Safty")
  sys.exit(0)

calibration_process = Calib(name)
